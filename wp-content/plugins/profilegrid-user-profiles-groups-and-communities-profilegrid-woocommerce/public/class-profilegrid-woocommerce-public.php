...
class Profilegrid_Woocommerce_Public {
    ...
    public function enqueue_scripts() {
     ...
     public function pg_purchases_tab($uid,$gid)
    {
        ...
        if($dbhandler->get_global_option_value('pm_enable_woocommerce','0')==1 && isset($options['woocommerce_purchases_tab']) && $options['woocommerce_purchases_tab']==1)
        {
            echo '<li class="pm-profile-tab pm-pad10"><a class="pm-dbfl" href="#pg-woocommerce_purchases">'. __('Purchases','profilegrid-woocommerce').'</a></li>';
        }
    }

    public function pg_show_purchases_tab_content($uid,$gid)
    {
        $this->enqueue_scripts();
         $this->enqueue_styles();
        $dbhandler = new PM_DBhandler;
        ...
        if($dbhandler->get_global_option_value('pm_enable_woocommerce','0')==1 && isset($options['woocommerce_purchases_tab']) && $options['woocommerce_purchases_tab']==1)
        {
            ...
            if(isset($options['woocommerce_max_product']) && $options['woocommerce_max_product']!='')
            {
              $numberofposts = intval($options['woocommerce_max_product']); 
            }
            else
            {
                $numberofposts = 10;
    ...
    public function pg_product_review_tab($uid,$gid)
    {
        $this->enqueue_scripts();
         $this->enqueue_styles();
        $dbhandler = new PM_DBhandler;
        $pmrequests = new PM_request;
        $options = maybe_unserialize($dbhandler->get_value('GROUPS','group_options',$gid,'id'));
        if($dbhandler->get_global_option_value('pm_enable_woocommerce','0')==1 && isset($options['woocommerce_reviews_tab']) && $options['woocommerce_reviews_tab']==1)
        {
            echo '<li class="pm-profile-tab pm-pad10"><a class="pm-dbfl" href="#pg-woocommerce_reviews">'. __('Product Reviews','profilegrid-woocommerce').'</a></li>';
        }
    }

    public function pg_show_product_review_tab_content($uid,$gid)
    {
        $this->enqueue_scripts();
         $this->enqueue_styles();
        $dbhandler = new PM_DBhandler;
    ...
    public function pg_my_orders_tab($uid,$gid)
    {
        ...
        $pmrequests = new PM_request;
        $options = maybe_unserialize($dbhandler->get_value('GROUPS','group_options',$gid,'id'));
        if($dbhandler->get_global_option_value('pm_enable_woocommerce','0')==1 && isset($options['woocommerce_orders_in_account']) && $options['woocommerce_orders_in_account']==1)
        {
    ...
    public function pg_my_billing_address_tab($uid,$gid)
    {
        ...
        $pmrequests = new PM_request;
        $options = maybe_unserialize($dbhandler->get_value('GROUPS','group_options',$gid,'id'));
        if($dbhandler->get_global_option_value('pm_enable_woocommerce','0')==1 && isset($options['woocommerce_billing_address_in_account']) && $options['woocommerce_billing_address_in_account']==1)
        {
    ...
    public function pg_my_shipping_address_tab($uid,$gid)
    {
        ...
        $pmrequests = new PM_request;
        $options = maybe_unserialize($dbhandler->get_value('GROUPS','group_options',$gid,'id'));
        if($dbhandler->get_global_option_value('pm_enable_woocommerce','0')==1 && isset($options['woocommerce_shipping_address_in_account']) && $options['woocommerce_shipping_address_in_account']==1)
        {
    ...
    public function profile_magic_check_woocommerce_show_total_spent($profile_id)
    {
       ...
       // if($row->is_group_leader!=0)$group_leader = $pmrequests->pg_get_group_leaders($profile_user_group);
        ...
        $access = false;
        if(isset($group_options['woocommerce_show_total_spent']) && $group_options['woocommerce_show_total_spent']==1)
        {    
            $access_level = $group_options['woocommerce_show_total_spent_permission'];
