...
class Profile_Magic_Notification {
     ...
     *      */
            ...
            else
            {
                ...
                foreach ( $notification as $db_notification )
                {
                    ...
                    case  'Message'             :  $data['pm_notify'][$id] = $this->pm_generate_message_notice($db_notification,$id);
                                                    break;
                    case  'Mcred_badge_earned'  :  $data['pm_notify'][$id] = $this->pm_generate_mycred_badge_earned_notice($db_notification,$id);
                                                    break;
        ...
        public function pm_generate_notification_without_heartbeat($loadnum=1){
            ...
            {
            ?>
                  <div class='pg-alert-warning pg-alert-info'><?php _e('Thats it for today. You are all caught up!','profilegrid-user-profiles-groups-and-communities'); ?></div>
            <?php 
            }
            else
            {
                    ...
                    foreach ( $notification as $db_notification )
                    {
                        ...
                        switch($type)
                        {
                            ...
                            case  'Mcred_badge_earned'  :  $data['pm_notify'][$id] = $this->pm_generate_mycred_badge_earned_notice($db_notification,$id);
                                                            break;
                            default                     : $data = __("no new notification","profilegrid-user-profiles-groups-and-communities");
                                                            break;
                        }
                    ...
                    echo' <div id="pm_load_more_notif" class="pm-dbfl" onclick="pm_load_more_notification('.$new_loadnum.')" >'.__("Load More..","profilegrid-user-profiles-groups-and-communities").'</div>';
                    }
...
/*-----------------NOTIFICATION CREATION FUNCTIONS------------------*/  
        ...
        public function pm_added_new_message_notification($rid,$sid,$message){
         ...
         public function pm_added_earned_new_badge_notification($uid,$previous_level,$new_level,$reward,$badge){
            $dbhandler = new PM_DBhandler;
            //$rid = get_post_field( 'post_author',$comment->comment_post_ID);
            $timestamp = current_time('mysql',true);
            $meta = array('previous_level'=>$previous_level,'new_level'=>$new_level,'reward'=>$reward,'badge'=>$badge);
            //add something to meta if you want
            $message = '';
            $meta= maybe_serialize($meta);
            $data = array('type'=>'Mcred_badge_earned','sid'=>$uid,'rid'=>$uid,'timestamp'=>$timestamp,'description'=>$message,'status'=>1,'meta'=>$meta);
            $arg = array('%s','%s','%s','%s','%s','%s','%s');
            $dbhandler->insert_row('NOTIFICATION', $data,$arg);
        }
...
//            $current_user_group = $pmrequests->profile_magic_get_user_field_value($current_uid,'pm_group');            
       ...
       <path d="M0 0h24v24H0z" fill="none"/>
                ...
                <div class="pm-notification-footer pm-dbfl"><div class="pm-notification-buttons"><a href="'.$permalink.'">'.__('View','profilegrid-user-profiles-groups-and-communities').'</a></div></div>
            </div>
        </div>';
...
//                     $this->pm_change_notification_status($id,2);
        ...
        public function pm_comment_notification_title($notification)
        {
            ...
            {
                case 'pg_groupwalls':
                    $title = __('New Comment on Wall','profilegrid-user-profiles-groups-and-communities');
                    break;
                case 'profilegrid_blogs':
                    $title = __('New Comment on Blog','profilegrid-user-profiles-groups-and-communities');;
                    break;
                case 'attachment':
                    $title = __('New Comment on Photo','profilegrid-user-profiles-groups-and-communities');;
                    break;
                default:
                    $title = __('New Comment','profilegrid-user-profiles-groups-and-communities');;
                    break;
            }
        ...
        public function pm_generate_friend_request_notice($db_notification,$id){
            ...
            if ( $user_exist === false ) {
                ...
                $sender_name = '';
                $sender_avatar = '';
                $description = __('This user is no longer registered.','profilegrid-user-profiles-groups-and-communities');
                $button = '';
            } else {
                //user id exists
                $profile_url = $pmrequests->pm_get_user_profile_url($notif_sender_id);
  ...
  //          $this->pm_change_notification_status($id,2);
    ...
    public function pm_generate_message_notice($db_notification,$id){
       ...
       <path d="M0 0h24v24H0z" fill="none"/>
                ...
                <div class="pm-notification-footer pm-dbfl"><div class="pm-notification-buttons"><a href="#pg-messages">'.__('View','profilegrid-user-profiles-groups-and-communities').'</a></div></div>
            </div>
        </div>';
  ...
  //          $this->pm_change_notification_status($id,2);
   ...
   public function pm_generate_mycred_badge_earned_notice($db_notification,$id){
       ...
       if(defined( 'myCRED_BADGE' ) ):
            ...
            $sender_avatar = $meta['badge']->main_image;
            $sender_name = $meta['badge']->title;
            $description = '';
